...
namespace adventOfCode2023
{
    class Day1
    {
        ...
        static int FindLastDigit(String str)
        {
        ...
        static int CalcLineValue(String line)
        {
            int val1 = FindFirstDigit(line);
            int vall2 = FindLastDigit(line);
            int total = val1 * 10 + vall2;
            return total;
        }

        static int CalcLineValue2(String line)
        {
            String[] strs = {
                "one",
                "two",
            ...
            for (int i = 0; i < line.Length; ++i)
            {
                ...
                {
                    int val = Int32.Parse(line[i].ToString());
                    if (firstNum == -1)
                    {
                        firstNum = val;
                    }
                ...
                else
                {
                    ...
                    {
                        String str = strs[j];
                        int count = line.Length - i;
                        int at = line.IndexOf(str, i, count);
                        if (at == i)
                        {
                            int val = j + 1;
                            if (firstNum == -1)
            ...
            // there was only 1 number in the line
            if (lastNum == -1)
            {
                lastNum = firstNum;
            }
            int total = firstNum * 10 + lastNum;
            //Console.WriteLine("Line, first, last, total: " + line + ", " + firstNum + ", " + lastNum + ", " + total);
            return total;
        }

        static int CalcPart1(String[] inLines)
        {
            int sum = 0;
            foreach (String line in inLines)
            {
                sum += CalcLineValue(line);
            }
            return sum;
        }

        static int CalcPart2(String[] inLines)
        {
            int sum = 0;
            foreach (String line in inLines)
            {
                sum += CalcLineValue2(line);
            }
            return sum;
        }
        ...
        {
            int testPart1 = CalcPart1(testInput);
            if (testPart1 == 142)
            {
                Console.WriteLine("Part1 Test pass\n");
            }
            else
            {
                Console.WriteLine("Part1 test fail\n");
                return;
            }

            int part1Sum = CalcPart1(part1Input);
            Console.WriteLine("Part1 sum: " + part1Sum);
        }

        public static void Part2()
        {
            int testPart2 = CalcPart2(test2Input);
            if (testPart2 == 281)
            {
                Console.WriteLine("Part2 Test Pass\n");
            }
            else
            {
                Console.WriteLine("Part2 test fail\n");
                return;
            }

            int part2Sum = CalcPart2(part1Input);
