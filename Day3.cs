...
namespace adventOfCode2023
{
    class EnginePart
    {
        ...
        public EnginePart(int _row, int _startCol, int _endCol)
        {
            row = _row;
            startCol = _startCol;
            endCol = _endCol;
        ...
        public int GetValue(String[] inLines)
        {
            int multiplier = 1;
            int sum = 0;
            for (int col = endCol; col >= startCol; col--)
            {
                sum += Int32.Parse(inLines[row][col].ToString()) * multiplier;
                multiplier *= 10;
            }
            return sum;
        }

        public bool IsEqual(EnginePart other)
        {
    ...
    class Day3
    {
        ...
        public static List<EnginePart> ParseInput(String[] inLines)
        {
            ...
            foreach (String line in inLines)
            {
                ...
                while (i < line.Length)
                {
                    if (line[i] >= '0' && line[i] <= '9')
                    {
                        ...
                        parts.Add(new EnginePart(row, startCol, endCol));
                    }
                    else
                    {
                        ++i;
        ...
        public static bool TestEnginePart(String[] inLines, EnginePart part)
        {
            int row = part.row;
            int col = part.startCol;
            for (int r = row - 1; r <= row + 1; ++r)
            {
                if (r < 0 || r >= inLines.Length)
                {
                    continue;
                }
                for (int c = col - 1; c <= part.endCol + 1; ++c)
                {
                    if (c < 0 || c >= inLines[0].Length)
                    {
                        continue;
                    }
                    if (inLines[r][c] != '.' && !(inLines[r][c] >= '0' && inLines[r][c] <= '9'))
                    {
                        // found symbol
                        return true;
                    }
        ...
        public static List<int> GetValidPartNums(String[] inLines, List<EnginePart> parts)
        {
            List<int> validParts = new List<int>();
            foreach (EnginePart part in parts)
            {
                if (!TestEnginePart(inLines, part))
                {
                    continue;
                }
                validParts.Add(part.GetValue(inLines));
            }
            return validParts;
        }

        public static int GetValidPartsSum(String[] inLines)
        {
            List<EnginePart> parts = ParseInput(inLines);
            List<int> validPartNums = GetValidPartNums(inLines, parts);
            return validPartNums.Sum();
        }

        public static int GetGearRatiosSum(String[] inLines)
        {
            int ratioSum = 0;
            List<EnginePart> parts = ParseInput(inLines);
            List<EnginePart> validParts = new List<EnginePart>();
            foreach (EnginePart part in parts)
            {
                if (TestEnginePart(inLines, part))
                {
            ...
            List<EnginePart> foundParts = new List<EnginePart>();
            for (int row = 0; row < inLines.Length; ++row)
            {
                for (int col = 0; col < inLines[row].Length; ++col)
                {
                    if (inLines[row][col] != '*')
                    {
                        continue;
                    ...
                    // look for two valid parts around this *
                    foundParts.Clear();
                    for (int r = row-1; r <= row+1; ++r)
                    {
                        for (int c = col-1; c <= col+1; ++c)
                        {
                            foreach (EnginePart part in validParts)
                            {
                                // matching surrounding part
                                if (part.row == r && c >= part.startCol && c <= part.endCol)
                                {
                                    // make sure we don't add the same part more than once
                                    bool foundExisting = false;
                                    foreach (EnginePart existing in foundParts)
                                    {
                                        if (existing.IsEqual(part))
                                        {
                                            foundExisting = true;
                                            break;
                                        }
                    ...
                    {
                        // valid gear symbol
                        int ratio = foundParts[0].GetValue(inLines) * foundParts[1].GetValue(inLines);
                        ratioSum += ratio;
                    }
                }
            }
        ...
        {
            if (GetValidPartsSum(testInput) != 4361)
            {
            ...
            int part1Sum = GetValidPartsSum(actualInput);
            Console.WriteLine("Part1 sum: " + part1Sum);
        }

        public static void Part2()
        {
            if (GetGearRatiosSum(testInput) != 467835)
            {
            ...
            int part2Sum = GetGearRatiosSum(actualInput);
            Console.WriteLine("Part2 sum: " + part2Sum);
        }
    }
}
